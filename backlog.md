...
**The skeleton builds and is tested; nothing else exists.** Since B-01 there is a `:server` module
producing a JVM jar and a `linuxX64` executable, one route, the kore wiring and the size gate; since
B-06 there are 12 tests on each target from one source, a CI job that compiles the code at all, and a
report step that fails when a **named** target's suite goes missing. There is no store (B-02), no JVM
distribution (B-03) and no image (B-04), and `linuxArm64` is covered by nobody (B-15).
