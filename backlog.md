...
**The skeleton builds; nothing else exists.** Since B-01 there is a `:server` module that produces a
JVM jar and a `linuxX64` executable, one route, the kore wiring and the size gate. There is no store
(B-02), no JVM distribution (B-03), no image (B-04) and no test (B-06) — `./gradlew build` today runs
**zero tests**, which is precisely the green-build-that-visited-nothing that B-06 exists for.
...
| [B-14](docs/backlog/B-14-configuration-cache-back-on.md) `[~]` | Take sborka 0.4.0.80 and delete the configuration-cache workaround | P1 | S | - |
